...
def test_draw_c_writes_per_model_stems_never_mixed(tmp_path):
    ...
    stems = {path.stem for path in tmp_path.glob("fig-c*")}
    assert "fig-c-vicsek" in stems
    assert "fig-c-votante" in stems
    assert "fig-c" not in stems
    assert not (tmp_path / "fig-c.png").exists()


def test_draw_c_writes_singles_when_multiple_rhos(tmp_path):
    draw_c(_agg_both_models(), fig_dir=tmp_path)
    assert (tmp_path / "fig-c-vicsek-rho2.png").is_file()
    assert (tmp_path / "fig-c-vicsek-rho4.png").is_file()
...
def test_draw_d_time_axvline_uses_usable_onset(tmp_path):
    ...
    assert (tmp_path / "fig-d-S-t-vicsek.png").is_file()
    assert not (tmp_path / "fig-d-S-t.png").exists()
...
def test_save_keeps_decimal_eta_in_filename(tmp_path):
    fig, ax = plt.subplots()
    ax.plot([0, 1], [0, 1])
    stem = tmp_path / "fig-b-vicsek-rho4-eta0.5"
    real_save(fig, stem)
    assert (tmp_path / "fig-b-vicsek-rho4-eta0.5.png").is_file()
    assert (tmp_path / "fig-b-vicsek-rho4-eta0.5.pdf").is_file()
    assert not (tmp_path / "fig-b-vicsek-rho4-eta0.png").exists()
